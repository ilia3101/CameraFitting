 ...
 * can be used to represent a sparse (well, blocky) jacobian matrix for bundle adjustment.
 */
#[derive(Clone, Debug)]
pub struct Block<T> {
...
impl<T: FloatCast> FloatCast for Parameter<T> {
    #[inline] fn from_f64(x: f64) -> Self { Self::locked(T::from_f64(x)) }
    #[inline] fn from_f32(x: f32) -> Self { Self::locked(T::from_f32(x)) }
    #[inline] fn from_i64(x: i64) -> Self { Self::locked(T::from_i64(x)) }
    #[inline] fn from_u64(x: u64) -> Self { Self::locked(T::from_u64(x)) }
    ...
    #[inline] fn as_usize(self) -> usize { self.value.as_usize() }
    #[inline] fn as_isize(self) -> isize { self.value.as_isize() }
}
...
pub trait ToParameters<T: Float>: Mappable<T> {
    #[inline] fn to_pars(self) -> Self::Wrapped<Parameter<T>> { self.fmap(|v| Parameter::locked(v)) }
    #[inline] fn to_pars_unlocked(self) -> Self::Wrapped<Parameter<T>> { self.fmap(|v| Parameter::unlocked(v)) }
}
impl<M, T: Float> ToParameters<T> for M where M: Mappable<T> {}

/* Conver back to T */
pub trait FromParameters<T: Float>: Mappable<Parameter<T>> {
    #[inline] fn from_pars(self) -> Self::Wrapped<T> { self.fmap(|p| p.value) }
}
impl<M, T: Float> FromParameters<T> for M where M: Mappable<Parameter<T>> {}
...
where
    Self: Mappable<P> + Clone, P: GeneralParameter<T>
{
