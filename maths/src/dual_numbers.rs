...
// TODO: implement each method properly here
impl<T: FloatCast, U: Zero> FloatCast for Dual<T,U> {
    #[inline] fn from_f64(x: f64) -> Self { Dual(T::from_f64(x), U::zero()) }
    #[inline] fn from_f32(x: f32) -> Self { Dual(T::from_f32(x), U::zero()) }
    #[inline] fn frac(t: i64, b: u64) -> Self { Dual(T::frac(t,b), U::zero()) }
    #[inline] fn int(x: i32) -> Self { Dual(T::int(x), U::zero()) }
    #[inline] fn from_i64(x: i64) -> Self { Dual(T::from_i64(x), U::zero()) }
    #[inline] fn from_u64(x: u64) -> Self { Dual(T::from_u64(x), U::zero()) }
    ...
    #[inline] fn as_usize(self) -> usize { self.x.as_usize() }
    #[inline] fn as_isize(self) -> isize { self.x.as_isize() }
}
